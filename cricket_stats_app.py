...
def main():
    ...
    # Title and top-level layout
    st.markdown("<h1 style='text-align: center; color: #333333; font-size:50px; font-family:Arial, sans-serif; font-weight:bold;'>NVCC Player Stats</h1>", unsafe_allow_html=True)
    ...
    if not st.session_state.excel_file:
        try:
            ...
            st.session_state.excel_sheets = relevant_sheets
            ...
            st.error(f"Failed to load file '{excel_file_path}': {e}")
            return
    ...
    if st.session_state.excel_file:
        ...
        col_year = st.columns([1,3,1])[1]
        with col_year:
            selected_year = st.selectbox("Select Year:", year_options, index=year_options.index(st.session_state.year_var))
        ...
        tabs = st.tabs(["Player Comparison", "Custom Report Generator", "Player Stats Over Years", "Top Players Over Years"])
        ...
        with tabs[2]:
            # Player Stats Over Years
            player_stats_over_years(st.session_state)

        with tabs[3]:
            # Top Players Over Years
...
def apply_custom_styles():
    st.markdown("""
        ...
        .st-radio>div>label, .st-checkbox>div>label {
            font-size: 1.1em !important;
        }
        </style>
...
def player_comparison(session_state):
    st.markdown(f"<h2 style='color: #2c3e50; font-weight:bold;'>{session_state.year_var} Career Analysis</h2>", unsafe_allow_html=True)
    ...
    with st.expander("Select Stats to Display", expanded=False):
        available_stats = [col for col in session_state.stats_data.columns if col != 'Player']
        selected_stats_display = [stat_display_names.get(stat, stat) for stat in session_state.selected_stats]
        chosen_stats = st.multiselect("Select Stats:", [stat_display_names.get(stat, stat) for stat in available_stats],
                                      default=selected_stats_display)
        # Map display names back to actual stat names
        reverse_stat_display_names = {v: k for k, v in stat_display_names.items()}
        session_state.selected_stats = [reverse_stat_display_names.get(stat, stat) for stat in chosen_stats]

    # Player Selection
    st.markdown("<h3 style='color: #2c3e50;'>Player Selection</h3>", unsafe_allow_html=True)
    col_p1, col_p2, col_p3 = st.columns(3)
    with col_p1:
        search_text1 = st.text_input("Search Player 1:")
        player_options1 = [player for player in session_state.player_list if search_text1.lower() in player.lower()]
        selected_player1 = st.selectbox("Select Player 1:", player_options1)
    with col_p2:
        compare_player2 = st.checkbox("Compare with second player")
        if compare_player2:
            search_text2 = st.text_input("Search Player 2:")
            player_options2 = [player for player in session_state.player_list if search_text2.lower() in player.lower()]
            selected_player2 = st.selectbox("Select Player 2:", player_options2)
        else:
            selected_player2 = None
    with col_p3:
        compare_player3 = st.checkbox("Compare with third player")
        if compare_player3:
            search_text3 = st.text_input("Search Player 3:")
            player_options3 = [player for player in session_state.player_list if search_text3.lower() in player.lower()]
            selected_player3 = st.selectbox("Select Player 3:", player_options3)
        else:
...
def custom_report_generator(session_state):
    st.markdown("<h2 style='color: #2c3e50; font-weight:bold;'>Custom Report Generator</h2>", unsafe_allow_html=True)
...
def generate_custom_report(stats_data, stat, sort_order, selected_additional_stats, one_decimal_stats,
    ...
    columns_to_display = list(dict.fromkeys(columns_to_display))  # Remove duplicates
...
def player_stats_over_years(session_state):
    st.markdown("<h2 style='color: #2c3e50; font-weight:bold;'>Player Stats Over Years</h2>", unsafe_allow_html=True)
...
def top_players_over_years(session_state):
    st.markdown("<h2 style='color: #2c3e50; font-weight:bold;'>Top Players Over Years</h2>", unsafe_allow_html=True)
        ...
        if sheet != "ALL TIME" and sheet != "Career Achievements":
            ...
            for add_stat in available_additional_stats:
                ...
                if pd.isna(add_value):
                    ...
                    add_value = format_value(add_value)
                add_values.append(f"{stat_display_names.get(add_stat, add_stat)}: {add_value}")
            additional_stats_values.append(add_values)
